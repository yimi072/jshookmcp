...
async function handleExtMessage(msg, sender) {
  ...
  if (msg.cmd === 'network_reload_capture') return await handleNetworkReloadCapture(msg, sender);
  if (msg.cmd === 'tabs') {
    try {
...
/**
 * Attach debugger, enable Network domain, hard-reload the tab, wait for requests to settle,
...
async function handleNetworkReloadCapture(msg, sender) {
  ...
  const ignoreCache = msg.ignoreCache !== false;
  /** @type {{ url: string, method?: string, postData?: string, requestId?: string }[]} */
  const captured = [];
  ...
  try {
    ...
    await chrome.debugger.sendCommand({ tabId }, 'Page.reload', { ignoreCache });

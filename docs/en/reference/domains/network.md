...
| `cdp_batch` | Run a batch of Chrome DevTools Protocol commands on the current page. Supports $N.path references to access results from previous commands in the batch. |
| `network_reload_capture` | Attach Chrome debugger, enable Network domain, reload the page, wait for requests to settle, then return captured requests matching the URL substring. Useful for capturing encrypted API payloads. |
